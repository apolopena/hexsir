...
def verify_cmd(file: str, offset: int) -> None:
    ...
    if computed_mutated != stored_original:
        ...
        stored_bytes = data[checksum_offset : checksum_offset + 4]
        value_hex = stored_bytes.hex().upper()
        print()
        header("Checksum Spec")
        info(f"offset: {checksum_offset}")
        info("length: 4")
        info(f"value:  {value_hex}")
        print()
        info(f"HEXSIR_CKSPEC: offset={checksum_offset},length=4,value={value_hex}")
    else:
