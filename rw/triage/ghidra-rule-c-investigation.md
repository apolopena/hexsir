...
**Status:** PARTIAL — Rule C narrowed to a specific code path but exact cap source not pinpointed. All function renames + comments applied to the Ghidra project so the trail persists across sessions.
**Created:** 2026-04-29
**Session:** Claude Code with `mcp__ghidra` (HTTP MCP at `127.0.0.1:8080/mcp`).

## Sources

- `rw/triage/items-add-primitive-cap.md` — parent triage with empirical Rule A/B/C data and prior Codex-driven Ghidra findings.
...
- `rw/triage/items-add-primitive-cap.md` — empirical Rule A/B/C data and bisect history.
- `rw/key-findings/magical-objects.md` — record format, edit primitives, engine validation summary.
