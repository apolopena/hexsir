...
**Status:** confirmed
**Status notes:** verified end-to-end. Slots 1–5 swaps via the chapter-2 `laser-lenses_1` proof (talent-only swap to Trait Twins, rarity-only edit to Legendary, combined edit). Slots 6–10 swap verified 2026-05-03 via the epilogue `laser-lenses_1` proof (slot 6 → Twin Dummies, in-game HUD readback). Hero-specific verification is Geppetto only; cross-hero generality unverified.
**Created:** 2026-04-27
...
The picks block is `[u32 M][M × 16-byte talent GUIDs]` followed by the fixed `[8 zero][float][8 zero][close marker]` trailing region — the same trailing region [`../docs/workflow/cross-cutting.md`](../docs/workflow/cross-cutting.md) describes for the run-state record body. Talent-domain specifics:

- **Slot layout.** Slots 1..N are laid out back-to-back at 16-byte intervals starting at the byte after the picks count u32.
- **Verified picks counts and trailing floats across Geppetto proofs** (2026-05-03, `rw/scripts/inspect_picks_block.py`):
...
- `rw/findings/talents.md` — talent name reference (player-facing names).
- `rw/findings/save-binary-format.md` — overall save format conventions (CRC32, record markers).
