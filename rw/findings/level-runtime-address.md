...
> **ARCHIVED 2026-05-03** — level runtime address folded into OE listener finding.
> Folded into [`oe-dynamic-listener-data.md`](oe-dynamic-listener-data.md). Level runtime address discovery was part of the broader live-state mirror investigation.
...
The class identification, vtable RVAs, instance layout, and per-snap census were folded into key-findings: see `rw/findings/oe-dynamic-listener-data.md`. Per the rule that triage holds only unresolved items, those findings are not duplicated here.
