...
- **Item picker / chest / Sandman shop RNG harness.** Clone `tools/frida/force_seed.js` for non-talent picker entry functions. Mechanical once each entry RVA is known. Per `rw/triage/random-seed-system.md`.
- **Save prep/serializer function (Frida).** Hook `oCMemoryBinaryStream::Write` (image+0x5257d0) with a counter, log `Thread.backtrace()` on call #1 only — captures the chapter-end serializer entry that Ghidra's static xrefs can't see. Per `tools/frida/HANDOFF.md`.
- **GUID hash-tail reversal.** The 4-byte tails of save-record keys (Level: `5793e600`, etc.) are likely a hash. Crack the hash function so we can mint arbitrary save-record keys for asset paths. Per `rw/triage/save-guid-hash-tail.md`.
- **Live source-of-truth pin identity.** All known stat pins are mirrors, not authoritative. Trace `IDynamicValueListenerData` callback chain backwards from any listener instance. Per `rw/triage/pin-identity-uncertain.md`.
- **Remaining held resources.** Wood, bean, and other ingredient-type held resources don't have located fields. Walk HC/CRP serde to find them. Per `rw/triage/save-mint-status.md` item 2.
...
- **Romeo Mint promotion.** Move the verified lab `rw/saves/edits/lab/geppetto/laser-lenses_1/romeo-mint-ch1-all10-legendary__from-laser-lenses_1-proof/` to `rw/saves/mints/geppetto/chapter2/laser-lenses_1/romeo-pickscount0-all10-legendary/` and write the `info.md` per the new mint convention.
