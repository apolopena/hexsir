...
**Status:** active — canonical state-of-knowledge for Ravensmith save-file editing.
**Created:** 2026-04-27

Save-file editing is the active track. Live-memory work is **tabled, not closed** — see "Live memory: tabled" below and the archived findings under `rw/key-findings/archive/` for the technical record. Read-only live monitoring may return as a future track if per-session ASLR re-discovery (`rs discover` / structural anchoring) is built; live trainer editing is not on the near roadmap.
...
The segmentation of the non-ASCII portion (separator vs hash vs interned ID) is not yet determined — see "Open questions" and `rw/triage/save-guid-hash-tail.md`.
...
4. Write the result as `Profile_1.ob` (filename is forced — game expects exactly this; encode variant identity in the directory path per `rw/docs/playbook.md`).
...
- **Items / talents / abilities** — no clear storage pattern as GUID+value records. Likely length-prefixed inventory arrays of asset-id refs.
...
- **Hash-tail reversal.** Trailing bytes of save-record GUIDs may encode a hash of the asset path. If reversed, this unlocks programmatic GUID minting from the asset tree → potentially the path to item editing. See `rw/triage/save-guid-hash-tail.md` for the experiment plan.
...
- **A `rs discover` workflow** — interactive value-progression discovery + multi-address watch over a Session — would make per-session re-discovery practical (~5–10 min user cost per session). Scoped as a future track in `rw/triage/live-state-mirror-cascade.md` but not built.
...
For the full technical record (template specializations, vtable RVAs, RTTI-walk methodology, listener heap-scan technique, mirror tier classification, pin-identity hypothesis space), see the archived findings: [`rw/key-findings/archive/oe-dynamic-listener-data.md`](archive/oe-dynamic-listener-data.md).
...
- `rw/triage/geppetto-save-analysis.md` — original GUID survey + the unfound-stats list (in-run dream shards, stars of fate, items, abilities). Some entries are now superseded by this key finding; the unfound stats remain open.
- `rw/triage/save-guid-hash-tail.md` — hash-tail reversal hypothesis + experiment plan.
- `rw/triage/level-runtime-address.md` — historical: live-memory pin of Level. Superseded for the trainer-edit use case but useful as method record.
- `rw/triage/pin-identity-uncertain.md` — hypothesis space for what pinned listeners actually are.
- `rw/triage/live-state-mirror-cascade.md` — mirror tier classification + the `rs discover` proposal.

### Archive

- `rw/key-findings/archive/oe-dynamic-listener-data.md` — full live-memory work record.
- `rw/key-findings/archive/save-chapter-counter.md` — original chapter-counter writeup.
