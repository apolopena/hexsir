...
## Done
<!-- DONE_START -->
EXPERIMENTAL-3: Per-run "Level reached" reset — accumulated XP zeroing (2026-04-30)
  - found and zeroed accumulated XP at `oCDtEntityCpntGroupLevelPersistentData` body+0x15 (u32, byte-misaligned, sat next to the in-run hero level u32 at +0x11 that `rerw --level` writes)
  - empirically established the per-run "Level reached" formula as `effective_level = hero_level + XP / xp_threshold`: ch2 proof showed 5.54 (lvl=5, XP=2690), `--level 1` alone produced 7.73 on defeat (= 1 + ~6.73 from leftover XP), `--level 1` + XP=0 produced the correct ~5.x baseline matching the saved hero level
  - re-promoted golden `rw/saves/edits/golden/geppetto/chapter2/laser-lenses_1/zero-scores-and-level1/Profile_1.ob` (CRC `0xDA0EFBA5`) layered on the v2_level1 base + XP zero; old golden was CRC `0x5A09DBF5`
  - updated `rw/saves/edits/golden/geppetto/chapter2/laser-lenses_1/zero-scores-and-level1/breakthrough.md` (build chain step 7 = XP zero, final CRC, what-was-verified) and `rw/key-findings/save-edit-pipeline-2026-04-30.md` (GroupLevel +0x15 = XP, stat-source mapping table, "Level reached formula" gotcha replaces the prior "not located" note)
  - added a new rule to `CLAUDE.md`: "Save-edit lab base rule — never layer on a failed experiment" (new edits layer only on goldens or verified-success proofs, never on failed lab variants)
  - per-hero lifetime sum that aggregates each run's effective level across runs still TBD — likely in `oCDtHeroProfileData`; deferred. Per-run input to that sum is now fully controllable
  ...
  - **Mint POC — chapter-2 advanced state rolled back to chapter 1, played forward to a new chapter-2 save**: full round-trip pipeline. New goldens: `golden/geppetto/chapter1/zero-scores-stars7-ch1/` (the minted starting save built with v4 zero-out + Stars=7 + `rerw --chapter 0`, CRC `0xC14F2CBB`); `golden/geppetto/chapter2/2for1-thru-with-inventory/` (the chapter-2 follow-on after playing the chapter-1 minted golden through to chapter-2 boss kill, with held inventory keys/feathers/bean/extra dream-shards). Both have `breakthrough.md` + screenshots. Three things still uncontrolled and documented as known limitations: (1) held-inventory byte location unmapped despite empirical persistence, (2) "Raven Feathers consumed" stat stuck at 4 — `body+0x25` writes 0 don't take, mirror-cascade pattern, (3) chapter-achievement records source unmapped — drives a compounding-blanks bug on the score-details page where leftover invalid achievement slots interleave between chapter-1 and chapter-2 active icons
  ...
  - **Documentation + recipes**: added "Edit recipes" section to `rw/key-findings/save-edit-pipeline-2026-04-30.md` — copy/paste Python scripts for (a) Stars of Fate edit, (b) full mint chain, (c) diff two saves, (d) splice/insert into a body; documents the gaps in the mint recipe explicitly. Added new CLAUDE.md rules: "Saves are only generated at chapter-boss kills" (with the ~20-min real-time-play cost ceiling), "Ghidra: annotate findings on the spot" (rename functions / data / params / locals / structs / equates immediately, narrative WHY belongs in key-findings not Ghidra comments). Added the cipher-position confirmations and dedicated CLI design rules (the latter authored by another agent in parallel)
  - **Triage cleanup pass**: created new `rw/triage/save-mint-unresolved.md` consolidating the three open mint-recipe questions with hypothesis lists + discriminating tests; cross-references `live-state-mirror-cascade.md`. Trimmed conquered-content out of `rw/triage/geppetto-save-analysis.md` (CRC32, XP, Level, Chapter counters, Stars of Fate all moved out — now in key-findings) and `rw/triage/level-runtime-address.md` (vtable + class layout in `oe-dynamic-listener-data.md`). Archived `rw/triage/talent-record-discovery.md` to `rw/key-findings/archive/` (was CLOSED)
